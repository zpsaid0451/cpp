...
const int COMP_MAX = 100; //Maximum number for a computer based player (used to\
 randomly choose a number between 40 - 100 to see if succesfully get passed obs\
tacle)
const int COMP_MIN = 40; //Minimum number for computer based player (used to ra\
ndomly choose a number between 40 - 100 to see if succesfully get passed obstac\
le)
const int BASE_SKILL = 50; //Player's base chance of getting through obstacle
const int BASE_DIFFICULTY = 100; //Max value of the difficulty of an obstacle. \
At 100, this would be a random number between 1 and 100.
...
public:
 ...
 Race(); //Default Constructor
  ...
  // 4. Calls RaceOptions and calls GetStatus or calls ProgressRace (passes the\
 user's choice)
  // 5. Checks to see if someone won (their location exceeds the length of the \
race)
  // 6. Displays the winner of race. If human player wins, increases a stat and\
 increases wins
 ...
 // 2. Randomly assigns (1 - 100) difficlty to all obstacles (regardless of lo\
cation,
  //    all players have the same level difficulty for each round)
  // 3. Compares the BASE_SKILL + (5 * stat) to the obstacle difficulty to see \
if successfully
  //    navigated. Indicates if successful and increments location. If not succ\
essful,
  //    falls and increments falls
  // 4. Calls ProgressComputer to deal with computer
 // Preconditions - Player in race
  // Postconditions - Each racer either progresses to the next obstacle, or fal\
ls
  ...
  bool MainMenu();
  // Name: RaceOptions()
  // Desc - Displays the race menu (display status, choose speed, choose agilit\
y, or choose jump)
  // Preconditions - None
 // Postconditions - Returns integer of user choice (status = 1, speed = 2, ag\
ility = 3, jump = 4)
  int RaceOptions();
  // Name: ManageGame
  // Desc - Calls RaceTitle, then calls GetRacerInfo. Continually calls MainMen\
u
  //        until the user chooses quit then it thanks the user and quits
  // Preconditions - Game instantiated
  // Postconditions - Calls RaceTitle, then calls GetRacerInfo. Continually cal\
ls MainMenu
//                  until the user chooses quit then it thanks the user and q\
uits
  void ManageGame();
  // Name: ProgressComputer
  // Desc - Iterates over each computer player (players 2, 3, and 4) to see if \
they
  //        successfully navigate through an obstacle. Indicates if they make i\
t or fall.
  //        Compares a random number between COMP_MIN and COMP_MAX to the obsta\
cle difficulty
//        to see if they are successful
  // Preconditions - obstacleDifficulty is already calculated and passed to thi\
s function
  // Postconditions - For each computer player, they either make it and increme\
nt their
